...
checkModules mods = case checkProgram (mods, Data.Map.empty, Signature Data.Map.empty Data.Map.empty, 0) of
  Left err -> Left err
  Right (_,global_names,obs,_) -> pure ("\nok, defined modules:\n" ++ intercalate "\n" (fmap (\(x,_,_) -> x) mods))
    --(showGlobalNames obs global_names)

checkProgram :: ([Module],GlobalNames,Signature,Int) -> Either Error ([Module],GlobalNames, Signature,Int)
checkProgram (glob @ ([], _,_,_)) = pure glob
checkProgram (mod:mods, names, obs, fresh) =
  checkModule mod [] (mods, names, obs, fresh) >>= checkProgram

checkModule :: Module -> [String] -> ([Module],GlobalNames,Signature,Int) -> Either Error ([Module],GlobalNames, Signature,Int)
checkModule (mod_name,imports,decls) in_progress globnobs = do
  ...
  pure (
    unverified',
    Data.Map.insert mod_name (internalNames st') global_names,
    signature st',
    newName st')